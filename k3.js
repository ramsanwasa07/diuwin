// DiuWin Official K3 3-Dice Lottery Engine
...
let currentBetSelection = null; // { type, choice, odds, displayName }

let userBetsThisRound = [];
let allUserBets = [];

// Odds Mapping for Totals
const TOTAL_ODDS = {
  3: 207.36, 4: 69.12, 5: 34.56, 6: 20.74,
  7: 13.83, 8: 9.88, 9: 8.30, 10: 7.68,
  11: 7.68, 12: 8.30, 13: 9.88, 14: 13.83,
  15: 20.74, 16: 34.56, 17: 69.12, 18: 207.36
};

// History Records
let k3HistoryRecords = [];
...
function showToast(msg, duration = 2500) {
  const toast = document.getElementById('k3Toast');
  if (!toast) return;
...
function loadCachedK3Data() {
  const cachedHist = localStorage.getItem('diuwin_k3_history');
  if (cachedHist) {
    try {
      k3HistoryRecords = JSON.parse(cachedHist);
      renderHistoryTable();
    } catch (e) {}
  }
  const cachedBets = localStorage.getItem('diuwin_k3_mybets');
  if (cachedBets) {
    try {
...
window.addEventListener('DOMContentLoaded', () => {
  loadCachedK3Data();
  initUser();
  syncK3StateWithServer();
  startCountdownTimer();
  renderHistoryTable();
  setupEventListeners();
  renderDicePips('die1', 1);
  renderDicePips('die2', 2);
  renderDicePips('die3', 3);
  setInterval(syncK3StateWithServer, 3500);
});

let lastK3HistoryKey = '';

async function syncK3StateWithServer() {
  try {
    const res = await fetch(`${API_BASE}/api/k3/state`);
    if (!res.ok) return;
    const data = await res.json();
    if (data.success) {
      if (data.period) {
        currentPeriod = data.period;
        const el = document.getElementById('k3PeriodNumber');
      ...
      if (data.history && data.history.length > 0) {
        ...
        if (topKey !== lastK3HistoryKey) {
          lastK3HistoryKey = topKey;
          k3HistoryRecords = data.history.map(h => ({
            period: h.period,
            d: h.dice || [1, 2, 3],
            sum: h.sum,
            size: h.size,
          ...
          try {
            localStorage.setItem('diuwin_k3_history', JSON.stringify(k3HistoryRecords));
          } catch (e) {}
          renderHistoryTable();
        }
      }
...
function renderDicePips(dieId, val) {
  const die = document.getElementById(dieId);
  if (!die) return;

  let pipsHtml = '';
  if (val === 1) {
    pipsHtml = `<span class="pip pip-center gold-pip"></span>`;
  } else if (val === 2) {
  ...
  die.innerHTML = `<div class="die-face">${pipsHtml}</div>`;
}
...
function updateTimerDisplay() {
  ...
  const minTen = document.getElementById('k3MinTen');
  const minOne = document.getElementById('k3MinOne');
  const secTen = document.getElementById('k3SecTen');
  const secOne = document.getElementById('k3SecOne');
  ...
  document.querySelectorAll('.odds-card, .prop-card').forEach(btn => {
...
async function handleRoundCompletion() {
  ...
  try {
    const res = await fetch(`${API_BASE}/api/k3/state`);
    const data = await res.json();
    if (data && data.lastOutcome && data.lastOutcome.dice) {
  ...
  setTimeout(() => {
    ...
    let d1, d2, d3, sum, size, parity;
    if (serverOutcome) {
      [d1, d2, d3] = serverOutcome.dice;
      sum = serverOutcome.sum;
      size = serverOutcome.size;
      parity = serverOutcome.parity;
    } else {
    ...
    renderDicePips('die1', d1);
    renderDicePips('die2', d2);
    renderDicePips('die3', d3);

    // Add to history
    k3HistoryRecords.unshift({
    ...
    if (k3HistoryRecords.length > 25) k3HistoryRecords.pop();
    try {
      localStorage.setItem('diuwin_k3_history', JSON.stringify(k3HistoryRecords));
    } catch (e) {}
    renderHistoryTable();
    ...
    userBetsThisRound.forEach(b => {
      ...
      if (b.type === 'total' && parseInt(b.choice) === sum) {
        won = true;
        mult = TOTAL_ODDS[sum] || 10.0;
      } else if (b.type === 'prop') {
        if (b.choice === 'Big' && size === 'Big') { won = true; mult = 1.92; }
        else if (b.choice === 'Small' && size === 'Small') { won = true; mult = 1.92; }
        else if (b.choice === 'Odd' && parity === 'Odd') { won = true; mult = 1.92; }
        else if (b.choice === 'Even' && parity === 'Even') { won = true; mult = 1.92; }
      }
      ...
      roundBetsSummary.push(`${b.displayName || b.choice} (₹${betAmt})`);
      ...
      syncBetWithBackend('k3', 'K3 3-Dice', b);
    });
    ...
    if (userHadBet) {
      ...
      showOutcomePopup(currentPeriod, [d1, d2, d3], sum, size, parity, isWin, totalWin, lossAmount, roundBetsSummary);
    ...
    userBetsThisRound = [];
    currentPeriod++;
    document.getElementById('k3PeriodNumber').textContent = currentPeriod;
  }, 900);
}

function showOutcomePopup(period, dice, sum, size, parity, isWin, winAmount, lossAmount, betsSummary) {
  const diceUnicode = ['⚀', '⚁', '⚂', '⚃', '⚄', '⚅'];
  const diceHtml = dice.map(d => `<span class="outcome-die">${diceUnicode[d - 1]}</span>`).join('');

  const sizeBg = size === 'Big' ? '#f59e0b' : '#3b82f6';
  const parityBg = parity === 'Odd' ? '#10b981' : '#ec4899';

  const detailsHtml = `
    <div class="outcome-dice-row">${diceHtml}</div>
    <span class="outcome-badge-pill" style="background: rgba(255,255,255,0.15); font-weight: 800;">Sum: ${sum}</span>
    <span class="outcome-badge-pill" style="background: ${sizeBg}">${size}</span>
    <span class="outcome-badge-pill" style="background: ${parityBg}">${parity}</span>
  `;
...
// ==================== BET DRAWER ====================
function openBetDrawer(type, choice, odds, displayName) {
  if (timeRemaining <= 5) {
    showToast('Round locked! Please wait for next draw.');
    return;
  }

  currentBetSelection = { type, choice, odds, displayName };
  activeBaseMoney = 10;
  activeQuantity = 1;
  activeDrawerMultiplier = 1;

  document.getElementById('drawerChoiceBadge').textContent = `K3 ${activeTimerDuration / 60} Min: ${displayName} (Odds ${odds})`;
...
function confirmBetPlacement() {
  ...
  const betRecord = {
    ...
    name: currentBetSelection.displayName,
  ...
  try {
    localStorage.setItem('diuwin_k3_mybets', JSON.stringify(allUserBets));
  } catch (e) {}
  renderMyBets();

  showToast(`Bet placed for ₹${total.toFixed(2)} on ${currentBetSelection.displayName}!`);
  closeBetDrawer();
}
...
async function loadMyBetsFromServer() {
  const userId = currentUser ? currentUser.id : 'usr_guest_demo';
  try {
    const res = await fetch(`${API_BASE}/api/games/my-bets?gameId=k3&userId=${encodeURIComponent(userId)}`, {
...
function renderHistoryTable() {
  const tbody = document.getElementById('k3HistoryTableBody');
  if (!tbody) return;
  tbody.innerHTML = '';

  const diceUnicode = ['⚀', '⚁', '⚂', '⚃', '⚄', '⚅'];

  k3HistoryRecords.forEach(r => {
    const tr = document.createElement('tr');
    const diceIcons = r.d.map(val => diceUnicode[val - 1]).join(' ');

    tr.innerHTML = `
      <td class="td-period">${r.period}</td>
      <td style="font-weight:900; color:#1e293b;">${r.sum}</td>
      <td style="color:${r.size === 'Big' ? '#f59e0b' : '#3b82f6'};">${r.size}</td>
      <td style="color:${r.parity === 'Odd' ? '#10b981' : '#8b5cf6'};">${r.parity}</td>
      <td>
        <span class="td-dice-icons">${diceIcons}</span>
      </td>
    `;
    ...
    item.innerHTML = `
      <div style="display:flex; justify-content:space-between; align-items:center;">
        <div>
          ...
          <span style="color:#64748b; font-size:11px; margin-left:6px;">Period: #${String(b.period).slice(-4)}</span>
        </div>
        ${badgeHtml}
      </div>
...
function setupEventListeners() {
  ...
  document.querySelectorAll('.time-tab').forEach(t => {
    t.addEventListener('click', () => {
      ...
      timeRemaining = activeTimerDuration;
      showToast(`Mode switched to ${min} Minute K3`);
    });
  });

  // Number Odds Cards (3 to 18)
  document.querySelectorAll('.odds-card').forEach(c => {
    c.addEventListener('click', () => {
      const num = c.dataset.choice;
      const odds = c.dataset.odds;
      openBetDrawer('total', num, odds, `Total ${num}`);
    });
  });
  ...
  document.querySelectorAll('.prop-card').forEach(c => {
    c.addEventListener('click', () => {
      ...
      const odds = c.dataset.odds;
      openBetDrawer('prop', choice, odds, choice);
    });
  });
  ...
  document.querySelectorAll('.cat-tab').forEach(t => {
    t.addEventListener('click', () => {
      ...
      showToast(`Category: ${t.textContent}`);
    });
  });
